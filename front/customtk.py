...
class App(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        self.geometry("{0}x{1}+0+0".format(self.winfo_screenwidth(),
                                           (self.winfo_screenheight() - self.winfo_screenheight() % 32)))
        self.state('zoomed')
        # root!
        ...
        self.bt_profile = customtkinter.CTkButton(self.left_side_panel, text="Profile",
                                                  command=lambda: self.profile(user.id))
        self.bt_profile.grid(row=2, column=0, padx=20, pady=10)
        self.bt_notifications = customtkinter.CTkButton(self.left_side_panel, text="Notifications",
                                                        command=lambda: self.notifications(user.id))
        self.bt_notifications.grid(row=3, column=0, padx=20, pady=10)
        if user.type == 3:
            ...
            self.bt_late_returns = customtkinter.CTkButton(self.left_side_panel, text="Late Returns",
                                                           command=lambda: self.late_returns(user.id))
            self.bt_late_returns.grid(row=5, column=0, padx=20, pady=10)

            self.bt_orders = customtkinter.CTkButton(self.left_side_panel, text="Approve Orders",
                                                     command=lambda: self.orders(user.id))
            self.bt_orders.grid(row=6, column=0, padx=20, pady=10)
    ...
    def manager(self):
        ...
        def order_stuff(self):
            ...
            textbox = customtkinter.CTkEntry(master=window, width=200, height=10, font=('Century Gothic', 12),
                                             placeholder_text="Enter item's name")
            textbox.pack(pady=10)
            textbox.configure(state='disabled')
            textbox2 = customtkinter.CTkEntry(master=window, width=100, height=10, font=('Century Gothic', 12),
                                              placeholder_text="Type")
            textbox2.pack(pady=10)
            textbox2.configure(state='disabled')
            textbox3 = customtkinter.CTkEntry(master=window, width=200, height=40, font=('Century Gothic', 12),
                                              placeholder_text="Enter item's description")
            ...
            button_confirm = customtkinter.CTkButton(window, text="Confirm",
                                                     command=lambda: confirm_order_stuff(window, textbox.get(),
                                                                                         slider.get(), textbox2.get(),
                                                                                         textbox3.get()) if now.hour < 22 and now.hour > 6 else CTkMessagebox(
                                                         icon='warning', title="Warning", option_1="Ok",
                                                         message="You can only order items before 5pm").get())
    ...
    def acquire_item(self):
        ...
        def combolicious(choice):
            slider2.configure(number_of_steps=supply_lst.get_supply_avl_by_name(choice),
                              to=supply_lst.get_supply_avl_by_name(choice))
            slider2.set(0)
        ...
        label_return.pack()
        date = [datetime.now()]
        btn_choose_date = customtkinter.CTkButton(master=window, text='choose date and time',
                                                  command=lambda: self.choose_date_time(date))
        btn_choose_date.pack()
        self.label_time = customtkinter.CTkLabel(master=window,
                                                 text=f"return time : {date[0].strftime('%Y-%m-%d %H:%M')}")
        ...
        slider2 = customtkinter.CTkSlider(window, from_=0, to=supply_lst.get_supply_avl_by_name(combo_item.get()),
                                          number_of_steps=supply_lst.get_supply_avl_by_name(combo_item.get()),
                                          command=slider_event3)
        slider2.set(0)
        slider2.pack()
    ...
    def approve_order(self):
        selected_item = self.table.selection()  # Get currently selected item
        if not selected_item:  # If no item is selected, just return
            print("No item selected.")
            return
        item_values = self.table.item(selected_item, "values")
        borrow_id = item_values[0]  # Assuming the borrow_id is in the first column
        response = requests.post(url + 'approveOrder', data={'borrow_id': borrow_id})
        if response.status_code == 200:
            result = response.json()
            if result['message'] == 'Order approved successfully':
                print("Order approved successfully")
                self.table.delete(selected_item)  # Remove the approved order from the table
            else:
                print("Failed to approve order")
        else:
            print("Failed to send approve request")
...
def create_table(self, type):
    ...
    if type == 'supply':
        ...
        self.toptitle = customtkinter.CTkLabel(self.right_dashboard, text="Supply Solutions - Late Returns \n",
                                               font=customtkinter.CTkFont(size=20, weight="bold"))
        self.toptitle.pack(pady=10)
        ...
        # Define the columns of the table
        self.table["columns"] = (
        "Borrow ID", "Item ID", "User ID", "Number of Items", "Borrow Date", "Expected Return", "Real Return")

        # Set the headings of the columns
        self.table.column("Borrow ID", width=100, anchor="center", stretch=True)
        self.table.heading("Borrow ID", text="Borrow ID")

        self.table.column("Item ID", width=100, anchor="center", stretch=True)
        self.table.heading("Item ID", text="Item ID")

        self.table.column("User ID", width=100, anchor="center", stretch=True)
        self.table.heading("User ID", text="User ID")

        self.table.column("Number of Items", width=100, anchor="center", stretch=True)
        self.table.heading("Number of Items", text="Number of Items")

        self.table.column("Borrow Date", width=100, anchor="center", stretch=True)
        self.table.heading("Borrow Date", text="Borrow Date")

        self.table.column("Expected Return", width=100, anchor="center", stretch=True)
        self.table.heading("Expected Return", text="Expected Return")

        self.table.column("Real Return", width=100, anchor="center", stretch=True)
        self.table.heading("Real Return", text="Real Return")

        # Get the current date
        now = datetime.now()

        response = requests.get(url + 'getLateReturns')
        late_returns = []
        if response.status_code == 200:
            result = response.json()
            if result['message'] == 'successful':
                late_returns = result['late_returns']

        # Add the late returns to the table
        for lr in late_returns:
            self.table.insert("", "end", values=(
            lr['borrow_id'], lr['item_id'], lr['user_id'], lr['num_of_items'], lr['borrow_date'], lr['expected_return'],
            lr['real_return']))
        ...
        self.toptitle = customtkinter.CTkLabel(self.right_dashboard, text="Supply Solutions - Pending Orders \n",
                                               font=customtkinter.CTkFont(size=20, weight="bold"))
        self.toptitle.pack(pady=10)
        ...
        response = requests.get(url + 'getPendingOrders')
        pending_orders = []
        if response.status_code == 200:
            result = response.json()
            if result['message'] == 'successful':
        ...
        # Add the pending orders to the table
