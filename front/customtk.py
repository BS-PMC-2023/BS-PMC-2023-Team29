...
class ManagerDashboard:
    def __init__(self):
        self.app = tk.Tk()
        self.app.title("Manager Dashboard")
        self.button_actions = []

    def add_button_action(self, text, action):
        self.button_actions.append((text, action))

    def show_notification(self, title, message):
        messagebox.showinfo(title, message)

    def run(self):
        for text, action in self.button_actions:
            button = tk.Button(self.app, text=text, command=action)
            button.pack()
